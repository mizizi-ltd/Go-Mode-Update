...
const RouteEngine = (() => {
  ...
  const showEmergencyPanel = () => {
    ...
    // Compact safety banner (tight 2 lines)
    const warn = document.createElement('div');
    warn.className = 'bg-red-50 border border-red-200 text-red-900 px-2.5 py-1.5 rounded text-[10px] font-medium leading-tight mb-2';
    warn.innerHTML = `<strong class="text-red-700 font-black uppercase text-[9px] tracking-wider block mb-0.5">Mizizi Safety Protocol</strong>Keep this panel loaded offline. Reach out immediately for any security or transit issues in Arusha.`;
    detailsBody.appendChild(warn);

    const wrapper = document.createElement('div');
    wrapper.className = 'space-y-1.5';

    // Helper to create a tight single-number row
    const makeRow = (title, subtitle, number, telNum, btnColor = 'bg-red-600 hover:bg-red-700') => {
      const card = document.createElement('div');
      card.className = 'bg-white border border-stone-200 rounded px-2.5 py-1.5 flex items-center justify-between shadow-2xs gap-2';

      const left = document.createElement('div');
      left.className = 'min-w-0 flex-1 leading-tight';

      const t = document.createElement('p');
      t.className = 'text-[9px] font-bold text-stone-400 uppercase tracking-wider truncate';
      t.textContent = title;
      left.appendChild(t);

      const n = document.createElement('p');
      n.className = 'text-[11px] font-black text-slate-800 truncate';
      n.textContent = subtitle ? `${subtitle} — ${number}` : number;
      left.appendChild(n);

      const btn = document.createElement('a');
      btn.href = `tel:${telNum}`;
      btn.className = `px-2 py-0.5 ${btnColor} text-white text-[9px] font-black uppercase rounded tracking-wider transition-colors flex-shrink-0`;
    ...
    // 1. Vetted Taxi Service
    wrapper.appendChild(makeRow('Vetted Taxi Service', 'ALMUS Willard', '+255 762 983 198', '+255762983198'));

    // 2. Tourist Police (No redundant title, simply number with Call button)
    wrapper.appendChild(makeRow('Tourist Police', '', '+255 734 294 116', '+255734294116'));

    // 3. Emergency Numbers (Compact 3-column dial row)
    const emerCard = document.createElement('div');
    emerCard.className = 'bg-white border border-stone-200 rounded px-2.5 py-1.5 shadow-2xs';
    emerCard.innerHTML = `
      <p class="text-[9px] font-bold text-stone-400 uppercase tracking-wider mb-1 leading-none">Emergency Numbers</p>
      <div class="grid grid-cols-3 gap-1 text-center">
        <a href="tel:112" class="py-1 px-1 bg-red-50 hover:bg-red-100 border border-red-200 rounded text-[10px] font-black text-red-700 flex items-center justify-center gap-0.5 transition-colors">112 <span class="text-[8px] font-medium text-red-500">(Gen)</span></a>
        <a href="tel:999" class="py-1 px-1 bg-red-50 hover:bg-red-100 border border-red-200 rounded text-[10px] font-black text-red-700 flex items-center justify-center gap-0.5 transition-colors">999 <span class="text-[8px] font-medium text-red-500">(Pol)</span></a>
        <a href="tel:118" class="py-1 px-1 bg-red-50 hover:bg-red-100 border border-red-200 rounded text-[10px] font-black text-red-700 flex items-center justify-center gap-0.5 transition-colors">118 <span class="text-[8px] font-medium text-red-500">(Fire)</span></a>
      </div>
    `;
    wrapper.appendChild(emerCard);

    // 4. Medevac - Air Ambulance (Consolidated 2 numbers in one box with respective call buttons)
    const medCard = document.createElement('div');
    medCard.className = 'bg-white border border-stone-200 rounded px-2.5 py-1.5 shadow-2xs space-y-1';
    medCard.innerHTML = `
      <p class="text-[9px] font-bold text-stone-400 uppercase tracking-wider leading-none">Medevac - Air Ambulance (24h)</p>
      <div class="flex items-center justify-between pt-0.5">
        <span class="text-[11px] font-black text-slate-800">+255 767 996 996</span>
        <a href="tel:+255767996996" class="px-2 py-0.5 bg-red-600 hover:bg-red-700 text-white text-[9px] font-black uppercase rounded tracking-wider transition-colors">Call</a>
      </div>
      <div class="flex items-center justify-between border-t border-stone-100 pt-1">
        <span class="text-[11px] font-black text-slate-800">+255 683 996 996</span>
        <a href="tel:+255683996996" class="px-2 py-0.5 bg-red-600 hover:bg-red-700 text-white text-[9px] font-black uppercase rounded tracking-wider transition-colors">Call</a>
      </div>
    `;
    wrapper.appendChild(medCard);

    // 5. Meserani Snake Park
    wrapper.appendChild(makeRow('Meserani Snake Park (Snake Bites)', '', '+255 754 440 800', '+255754440800'));

    // 6. Mizizi Support Desk
    wrapper.appendChild(makeRow('Mizizi Support Desk', '', '+255 764 225 389', '+255764225389', 'bg-emerald-700 hover:bg-emerald-800'));

    detailsBody.appendChild(wrapper);

    // Read More Button (slim, high visibility, directly underneath)
    const readMoreBtn = document.createElement('button');
    readMoreBtn.type = 'button';
    readMoreBtn.className = 'w-full py-1.5 px-3 bg-red-700 hover:bg-red-800 text-white font-black text-[10px] uppercase tracking-wider rounded flex items-center justify-center gap-1 shadow-2xs transition-colors cursor-pointer mt-2';
    readMoreBtn.textContent = 'Read Detailed Emergency Protocol';
    readMoreBtn.addEventListener('click', () => {
  ...
  const openEmergencyDetailOverlay = () => {
    openOverlay("Emergency Protocol — Arusha", (container) => {
      ...
      hero.className = 'bg-red-700 text-white rounded-md px-3.5 py-3 mb-3';
      hero.innerHTML = `
        <p class="text-[9px] font-black uppercase tracking-widest text-red-200 mb-0.5">Safety & Medical Protocol</p>
        <h2 class="text-base sm:text-lg font-black tracking-tight leading-tight mb-1">Emergencies & Medical Care in Arusha</h2>
        <p class="text-[11px] font-medium leading-snug text-red-100">
          While Arusha possesses numerous clinical centres, selecting a hospital with modern diagnostic machinery and dedicated trauma units is essential for rapid recovery. Public emergency telephone lines can be slow or unresponsive; you should always contact private emergency responders or dedicated hospitals directly.
        </p>
      `;
      container.appendChild(hero);

      // Section title helper (clean, no emojis)
      const sectionTitle = (title, colorClass = 'text-slate-800') => {
        const h = document.createElement('h3');
        h.className = `text-[11px] font-black uppercase tracking-wider ${colorClass} mt-3 mb-1.5 border-b border-stone-200 pb-1`;
        h.textContent = title;
        return h;
      };

      // ── 1. National Emergency Lines (Compact tiles) ──
      container.appendChild(sectionTitle('National Emergency Lines', 'text-red-700'));

      const emerGrid = document.createElement('div');
      emerGrid.className = 'grid grid-cols-3 gap-1.5 mb-3';
      [
        { label: 'General', num: '112' },
        { label: 'Police', num: '999' },
        { label: 'Fire Brigade', num: '118 / 114', tel: '118' }
      ].forEach(({ label, num, tel }) => {
        const tile = document.createElement('div');
        tile.className = 'bg-red-50 border border-red-200 rounded px-2 py-1.5 flex items-center justify-between';
        tile.innerHTML = `
          <div class="leading-tight min-w-0">
            <span class="text-xs font-black text-red-800 block">${num}</span>
            <span class="text-[8px] font-bold text-red-600 uppercase tracking-wider truncate block">${label}</span>
          </div>
          <a href="tel:${tel || num}" class="px-1.5 py-0.5 bg-red-600 hover:bg-red-700 text-white text-[8px] font-black uppercase rounded tracking-wider transition-colors ml-1">Call</a>
        `;
        emerGrid.appendChild(tile);
      });
      container.appendChild(emerGrid);

      // ── 2. Emergency Air Evacuation & Critical Response ──
      container.appendChild(sectionTitle('Emergency Air Evacuation & Critical Response', 'text-red-700'));

      const medevacCard = document.createElement('div');
      medevacCard.className = 'bg-white border border-stone-200 rounded-md p-3 shadow-2xs space-y-1.5 mb-3';
      medevacCard.innerHTML = `
        <div class="flex items-center justify-between flex-wrap gap-1 border-b border-stone-100 pb-1.5">
          <h4 class="text-xs font-black text-slate-800">Arusha Medevac</h4>
          <div class="flex items-center gap-1.5 flex-wrap">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold text-slate-700">+255 767 996 996</span>
              <a href="tel:+255767996996" class="px-1.5 py-0.5 bg-red-600 hover:bg-red-700 text-white text-[8px] font-black uppercase rounded tracking-wider transition-colors">Call</a>
            </div>
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold text-slate-700">+255 683 996 996</span>
              <a href="tel:+255683996996" class="px-1.5 py-0.5 bg-red-600 hover:bg-red-700 text-white text-[8px] font-black uppercase rounded tracking-wider transition-colors">Call</a>
            </div>
          </div>
        </div>
        <p class="text-[10px] font-bold text-stone-500">Arusha Airport (ARK) · 24-Hour Emergency Response</p>
        <p class="text-[11px] text-stone-600 leading-snug">
          The primary private medical evacuation and air ambulance service operating across Northern Tanzania's safari circuits and urban hubs. Their flight physicians and critical care paramedics specialise in emergency tourist transfers.
        </p>
        <div class="bg-amber-50 border border-amber-200 rounded px-2.5 py-1.5 text-[10px] text-amber-900 leading-snug font-medium">
          A dedicated 4-week tourist evacuation pass costs approximately $20 USD; this guarantees rapid aircraft dispatch and en-route stabilisation to an advanced trauma centre. (Note: In-hospital care and subsequent treatment must be cleared by your travel insurance policy).
        </div>
      `;
      container.appendChild(medevacCard);

      // ── 3. Specialist Emergency: Snake Bites ──
      container.appendChild(sectionTitle('Specialist Emergency: Snake Bites', 'text-amber-800'));

      const snakeCard = document.createElement('div');
      snakeCard.className = 'bg-white border border-stone-200 rounded-md p-3 shadow-2xs space-y-1.5 mb-3';
      snakeCard.innerHTML = `
        <div class="flex items-center justify-between flex-wrap gap-1 border-b border-stone-100 pb-1.5">
          <h4 class="text-xs font-black text-slate-800">Meserani Snake Park Medical Clinic</h4>
          <div class="flex items-center gap-1">
            <span class="text-[10px] font-bold text-slate-700">+255 754 440 800</span>
            <a href="tel:+255754440800" class="px-1.5 py-0.5 bg-red-600 hover:bg-red-700 text-white text-[8px] font-black uppercase rounded tracking-wider transition-colors">Call</a>
          </div>
        </div>
        <p class="text-[10px] font-bold text-stone-500">Meserani (approx. 40 km west of Arusha along Dodoma Road)</p>
        <p class="text-[11px] text-stone-600 leading-snug">
          The recognised regional specialist centre and central repository for antivenom in Northern Tanzania. They stock multiple varieties of polyvalent and monovalent antivenoms and provide expert triage protocols for venomous bites.
        </p>
        <div class="bg-red-50 border border-red-200 rounded px-2.5 py-1.5 text-[10px] text-red-900 leading-snug font-semibold">
          Call their hotline DIRECTLY & IMMEDIATELY for advice before or during transit; do not wait for the hospitals to do it for you.
        </div>
      `;
      container.appendChild(snakeCard);

      // ── 4. Recommended Private & Specialist Hospitals in Arusha ──
      container.appendChild(sectionTitle('Recommended Private & Specialist Hospitals in Arusha', 'text-slate-800'));

      // NSK Hospitals
      const nskCard = document.createElement('div');
      nskCard.className = 'bg-white border border-stone-200 rounded-md p-3 shadow-2xs space-y-1.5 mb-2.5';
      nskCard.innerHTML = `
        <div class="flex items-center justify-between flex-wrap gap-1 border-b border-stone-100 pb-1.5">
          <h4 class="text-xs font-black text-slate-800">NSK Hospitals</h4>
          <div class="flex items-center gap-1">
            <span class="text-[10px] font-bold text-slate-700">+255 626 221 331</span>
            <a href="tel:+255626221331" class="px-1.5 py-0.5 bg-red-600 hover:bg-red-700 text-white text-[8px] font-black uppercase rounded tracking-wider transition-colors">Call</a>
          </div>
        </div>
        <p class="text-[10px] font-bold text-stone-500">Industrial Road, Arusha · Open 24/7 for acute trauma & emergencies</p>
        <p class="text-[11px] text-stone-600 leading-snug">
          One of the most modern private facilities in central Arusha, offering comprehensive digital diagnostic imaging, an emergency wing, laboratory services, and surgical specialists.
        </p>
      `;
      container.appendChild(nskCard);

      // Arusha Pediatric & Maternal Care Centre (APMC)
      const apmcCard = document.createElement('div');
      apmcCard.className = 'bg-white border border-stone-200 rounded-md p-3 shadow-2xs space-y-1.5 mb-2.5';
      apmcCard.innerHTML = `
        <div class="flex items-center justify-between flex-wrap gap-1 border-b border-stone-100 pb-1.5">
          <h4 class="text-xs font-black text-slate-800">Arusha Pediatric & Maternal Care Centre (APMC)</h4>
          <div class="flex items-center gap-1">
            <span class="text-[10px] font-bold text-slate-700">+255 757 841 240</span>
            <a href="tel:+255757841240" class="px-1.5 py-0.5 bg-red-600 hover:bg-red-700 text-white text-[8px] font-black uppercase rounded tracking-wider transition-colors">Call</a>
          </div>
        </div>
        <p class="text-[10px] font-bold text-stone-500">Serengeti Road, Block E, House No. 98, Arusha</p>
        <p class="text-[11px] text-stone-600 leading-snug">
          Operated under the Institute for Child Healthcare Africa (ICHA), this is the leading family-focused clinic in town. Bring children and infants here for specialised paediatric consultations, emergency childhood triage, and maternal care.
        </p>
      `;
      container.appendChild(apmcCard);

      // Mount Meru Regional Referral Hospital (VIP Wing / Fast Track)
      const mmuCard = document.createElement('div');
      mmuCard.className = 'bg-white border border-stone-200 rounded-md p-3 shadow-2xs space-y-1.5 mb-2.5';
      mmuCard.innerHTML = `
        <div class="flex items-center justify-between flex-wrap gap-1 border-b border-stone-100 pb-1.5">
          <h4 class="text-xs font-black text-slate-800">Mount Meru Regional Referral Hospital (VIP Wing / Fast Track)</h4>
          <div class="flex items-center gap-1">
            <span class="text-[10px] font-bold text-slate-700">+255 748 535 480</span>
            <a href="tel:+255748535480" class="px-1.5 py-0.5 bg-red-600 hover:bg-red-700 text-white text-[8px] font-black uppercase rounded tracking-wider transition-colors">Call</a>
          </div>
        </div>
        <p class="text-[10px] font-bold text-stone-500">Hospital Road (opposite Arusha International Conference Centre - AICC), City Centre</p>
        <p class="text-[11px] text-stone-600 leading-snug">
          The largest regional referral hospital in Arusha. While the general public wards are very busy, their dedicated VIP / Fast-Track Concierge Wing provides expedited private consultations, private rooms, and coordinated emergency assistance.
        </p>
      `;
      container.appendChild(mmuCard);

      // ── 5. Vetted Taxi Service & Tourist Police ──
      container.appendChild(sectionTitle('Local Transit & Security Support', 'text-slate-800'));

      // Taxi
      const taxiCard = document.createElement('div');
      taxiCard.className = 'bg-white border border-stone-200 rounded-md p-3 shadow-2xs space-y-1.5 mb-2.5';
      taxiCard.innerHTML = `
        <div class="flex items-center justify-between flex-wrap gap-1 border-b border-stone-100 pb-1.5">
          <h4 class="text-xs font-black text-slate-800">ALMUS Willard — Kilimanjaro Taxi</h4>
          <div class="flex items-center gap-1">
            <span class="text-[10px] font-bold text-slate-700">+255 762 983 198</span>
            <a href="tel:+255762983198" class="px-1.5 py-0.5 bg-red-600 hover:bg-red-700 text-white text-[8px] font-black uppercase rounded tracking-wider transition-colors">Call</a>
          </div>
        </div>
        <p class="text-[10px] font-bold text-stone-500">Vetted Taxi Service</p>
        <p class="text-[11px] text-stone-600 leading-snug">
          Vetted and trusted private taxi service for safe transit across Arusha town, late-night hotel returns, and airport connections.
        </p>
      `;
      container.appendChild(taxiCard);

      // Tourist Police
      const policeCard = document.createElement('div');
      policeCard.className = 'bg-white border border-stone-200 rounded-md p-3 shadow-2xs space-y-1.5 mb-2.5';
      policeCard.innerHTML = `
        <div class="flex items-center justify-between flex-wrap gap-1 border-b border-stone-100 pb-1.5">
          <h4 class="text-xs font-black text-slate-800">Tanzania Tourist Police</h4>
          <div class="flex items-center gap-1">
            <span class="text-[10px] font-bold text-slate-700">+255 734 294 116</span>
            <a href="tel:+255734294116" class="px-1.5 py-0.5 bg-red-600 hover:bg-red-700 text-white text-[8px] font-black uppercase rounded tracking-wider transition-colors">Call</a>
          </div>
        </div>
        <p class="text-[10px] font-bold text-stone-500">Arusha Division</p>
        <p class="text-[11px] text-stone-600 leading-snug">
          Dedicated tourist protection department with English-speaking officers for prompt assistance regarding safety, theft, or security.
        </p>
      `;
      ...
      ruleBox.className = 'bg-emerald-50 border border-emerald-200 rounded-md p-3 my-3';
      ruleBox.innerHTML = `
        <p class="text-[9px] font-black uppercase tracking-widest text-emerald-800 mb-0.5">Rule of Thumb</p>
        <p class="text-[11px] text-emerald-950 font-medium leading-snug">
          For life-threatening injuries, proceed immediately to the nearest facility for basic stabilization. If transport allows, contact Arusha Medevac or direct your driver straight to NSK Hospital (for adult trauma) or APMC (for children).
        </p>
      `;
      container.appendChild(ruleBox);

      // ── 7. Mizizi Support Desk ──
      const supportCard = document.createElement('div');
      supportCard.className = 'bg-white border border-emerald-300 rounded-md p-3 shadow-2xs space-y-1.5 mb-2';
      supportCard.innerHTML = `
        <div class="flex items-center justify-between flex-wrap gap-1 border-b border-stone-100 pb-1.5">
          <h4 class="text-xs font-black text-emerald-900">Mizizi Support Desk</h4>
          <div class="flex items-center gap-1">
            <span class="text-[10px] font-bold text-slate-700">+255 764 225 389</span>
            <a href="tel:+255764225389" class="px-1.5 py-0.5 bg-emerald-700 hover:bg-emerald-800 text-white text-[8px] font-black uppercase rounded tracking-wider transition-colors">Call</a>
          </div>
        </div>
        <p class="text-[11px] text-stone-600 leading-snug">
          Reach out to the Mizizi team anytime for route assistance, emergency coordination, local language translation, or general safety inquiries.
        </p>
      `;
